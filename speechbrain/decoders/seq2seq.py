...
class S2SStackedTransformerBeamSearch(S2STransformerBeamSearch):
    ...
    def forward_step(
        self, inp_tokens, memory, enc_states, second_enc_states, enc_lens
    ):
        memory = _update_mem(inp_tokens, memory)
        pred, attn = self.model.decode(memory, enc_states, second_enc_states)
        prob_dist = self.softmax(self.fc(pred) / self.temperature)
        return prob_dist[:, -1, :], memory, attn

    def forward(self, enc_states, second_enc_states, wav_len):  # noqa: C901
        # mainly took from the S2STransformerBeamSearch
        # but this function add second representation as the input of the model
        enc_lens = torch.round(enc_states.shape[1] * wav_len).int()
        device = enc_states.device
        batch_size = enc_states.shape[0]
        ...
        # Inflate the enc_states and enc_len by beam_size times
        enc_states = inflate_tensor(enc_states, times=self.beam_size, dim=0)
        second_enc_states = inflate_tensor(
            second_enc_states, times=self.beam_size, dim=0
        )
        enc_lens = inflate_tensor(enc_lens, times=self.beam_size, dim=0)
        ...
        for t in range(max_decode_steps):
            ...
            log_probs, memory, attn = self.forward_step(
                inp_tokens, memory, enc_states, second_enc_states, enc_lens
            )
            log_probs = self.att_weight * log_probs
