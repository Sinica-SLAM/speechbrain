...
class S2SGlobalTransformerBeamSearch(S2STransformerBeamSearch):
    def __init__(self, modules, temperature=1.0, temperature_lm=1.0, **kwargs):
        super().__init__(
            modules, temperature, temperature_lm, **kwargs,
        )

    def forward(self, enc_states, wav_len, global_weight):  # noqa: C901
        enc_lens = torch.round(enc_states.shape[1] * wav_len).int()
        device = enc_states.device
        batch_size = enc_states.shape[0]
        ...
        for t in range(max_decode_steps):
            ...
            log_probs, memory, attn = self.forward_step(
                inp_tokens, memory, enc_states, enc_lens, global_weight,
            )
            log_probs = self.att_weight * log_probs
    ...
    def forward_step(
        self, inp_tokens, memory, enc_states, enc_lens, global_weight
    ):
        memory = _update_mem(inp_tokens, memory)
        pred, attn = self.model.decode(memory, enc_states, global_weight)
        prob_dist = self.softmax(self.fc(pred) / self.temperature)
        return prob_dist[:, -1, :], memory, attn
