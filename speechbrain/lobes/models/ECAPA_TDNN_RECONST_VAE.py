...
class ECAPA_TDNN(torch.nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList()
        self.reconst_layer = reconst_layer
    ...
    def __init__(self, channels, kernel_sizes, output_layer):
        ...
        for i in range(len(self.DecoderChannels) - 1):
            self.DecodeLayers.append(
                nn.Conv1d(
                    in_channels=self.DecoderChannels[i],
                    out_channels=self.DecoderChannels[i + 1],
                    kernel_size=kernel_sizes[i],
                    stride=1,
                    padding=floor(kernel_sizes[i] / 2),  # same
                )
            )
