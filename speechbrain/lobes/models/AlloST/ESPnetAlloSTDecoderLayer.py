...
class DecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if fusion_type == "stacked":
            ...
            self.gate = nn.Sequential(
                nn.Linear(size + size, size), nn.Sigmoid(),
            )
        else:
    ...
    def forward(
        self, tgt, tgt_mask, memory, memory_mask, phone, phone_mask, cache=None
    ):
        ...
        if self.concat_after:
            ...
            if self.fusion_type == "gate":
                ...
                g = self.gate(torch.cat((src_attn, p_attn), dim=-1))
        ...
        if self.fusion_type == "stacked":
            ...
            if self.concat_after:
                ...
                x_concat = torch.cat((x, attn), dim=-1)
