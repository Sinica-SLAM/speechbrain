...
from speechbrain.lobes.models.AlloST.ESPnetAlloSTDecoderLayer import (
    DecoderLayer,
)
...
class Decoder(BatchScorerInterface, torch.nn.Module):
    ...
    def forward(self, tgt, tgt_mask, memory, memory_mask, phone, phone_mask):
        """Forward decoder.
        Args:
        ...
        """
        x = self.embed(tgt)
        x, tgt_mask, memory, memory_mask, phone, phone_mask = self.decoders(
            x, tgt_mask, memory, memory_mask, phone, phone_mask,
        )
        if self.normalize_before:
            x = self.after_norm(x)
    ...
    def forward_one_step(self, tgt, tgt_mask, memory, phone, cache=None):
        """Forward one step.
        Args:
        ...
        for c, decoder in zip(cache, self.decoders):
            x, tgt_mask, memory, memory_mask, phone, phone_mask = decoder(
                x, tgt_mask, memory, None, phone, None, cache=c
            )
            new_cache.append(x)
