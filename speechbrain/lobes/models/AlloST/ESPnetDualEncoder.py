...
class DualEncoder(torch.nn.Module):
    ...
    def __init__(  # noqa: C901
        ...
        elif phone_embed_type == "GBST":
            from speechbrain.lobes.models.GBST import GBST
        ...
        if is_fusion:
            self.fusion_attn = MultiHeadedAttention(
                attention_heads, attention_dim, attention_dropout_rate,
            )
            self.fusion_norm = torch.nn.LayerNorm(attention_dim)
            self.fusion_dropout = torch.nn.Dropout(p=dropout_rate)
    ...
    def forward(
        ...
        if self.is_fusion:
            ...
            h_fusion = self.fusion_attn(xs, phone, phone, phone_mask)
            xs = xs + self.fusion_dropout(h_fusion)
            if not self.normalize_before:
                xs = self.after_norm(xs)

        return xs, masks, phone, phone_mask
