...
class Dual_Path_Model(nn.Module):
    ...
    def __init__(
        ...
        self.conv2d_mix = nn.Conv2d(
            out_channels, out_channels, kernel_size=1
        )
        self.end_conv1x1 = nn.Conv1d(out_channels, in_channels, 1, bias=False)
        self.end_conv1x1_mix = nn.Conv1d(out_channels, in_channels, 1, bias=False)
        self.prelu = nn.PReLU()
        ...
        self.output_gate_mix = nn.Sequential(
            nn.Conv1d(out_channels, out_channels, 1), nn.Sigmoid()
        )
    ...
    def forward(self, x, C=None):
        ...
        # [B, N, L]
        x_m = self._over_add(x_m, gap)
        x_m = self.output_mix(x_m) * self.output_gate_mix(x_m)
        # [B*spks, N, L]
        x = self._over_add(x, gap)
        x = self.output(x) * self.output_gate(x)

        # [B, N, L]
        x_m = self.end_conv1x1_mix(x_m)
        # [B*spks, N, L]
...
class Dual_Path_Model_pre(nn.Module):
    """The dual path model which is the basis for dualpathrnn, sepformer, dptnet.
    ...
    def __init__(
        ...
        super(Dual_Path_Model_pre, self).__init__()
        self.K = K
        self.num_spks = num_spks
        self.num_layers = num_layers
        self.norm = select_norm(norm, in_channels, 3)
...
class Dual_Path_Model_post(nn.Module):
    """The dual path model which is the basis for dualpathrnn, sepformer, dptnet.
    ...
    def __init__(
        ...
        super(Dual_Path_Model_post, self).__init__()
        self.num_spks = num_spks
...
class Cond_emb(nn.Module):
    def __init__(
        ...
        super(Cond_emb, self).__init__()
        use_affine = True if use_adain else use_affine
        num_nl_hidden = out_channels if use_adain else 1

        if dim_cond > 0:
            self.cond_b = nn.Linear(
                dim_cond, out_channels
            )
            self.cond_nl = nn.PReLU(
        ...
        if dim_cond > 0 and use_adain:
            self.cond_in = nn.InstanceNorm2d(out_channels)
        else:
            self.cond_in = None
    ...
    def forward(self, X, C=None):
        if C is None:
            ...
            Batch = C.size(0)
            Ca = (
                self.cond_a(C).view(Batch, -1, 1, 1).exp()
                if self.cond_a
                else 1.0
            )
            Cb = (
                self.cond_b(C).view(Batch, -1, 1, 1)
                if self.cond_b
                else 0.0
            )
            X = self.cond_in(X)
