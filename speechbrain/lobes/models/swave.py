    ...
    def forward(self, input):
        ...
        for i in range(self.num_layers):
            row_input = (
                ...
                .contiguous()
                .view(batch_size * d2, d1, -1)
            )
            row_output = self.rows_grnn[i](row_input)
            row_output = (
                row_output.view(batch_size, d2, d1, -1)
                .permute(0, 3, 2, 1)
                .contiguous()
            )
            ...
            col_input = (
                ...
                .contiguous()
                .view(batch_size * d1, d2, -1)
            )
            col_output = self.cols_grnn[i](col_input)
            col_output = (
                col_output.view(batch_size, d1, d2, -1)
                .permute(0, 3, 1, 2)
                .contiguous()
            )
...
class Separator(nn.Module):
    ...
    def pad_segment(self, input, segment_size):
        ...
        if rest > 0:
            pad = Variable(torch.zeros(batch_size, dim, rest)).type(
                input.type()
            )
    ...
    def forward(self, input):
        ...
        # print("enc_segments", enc_segments.shape)
        # separate
...
class Encoder(nn.Module):
    def __init__(self, L, N):
        ...
        # setting 50% overlap
        self.conv = nn.Conv1d(
            2, N, kernel_size=L, stride=L // 2, bias=False
        )  # Stereo
