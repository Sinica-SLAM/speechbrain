...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        ---------
        x : torch.Tensor
            Input tensor with dimensionality [B*spk, L].
        Return
        ------
        x : torch.Tensor
            Encoded tensor with dimensionality [B*spk, N, T_out].
...
class Dual_Computation_Block(nn.Module):
    ...
    def __init__(
        ...
        if dim_cond > 0:
            ...
            self.cond_b_intra = None
            self.cond_nl_intra = nn.Identity()
            self.cond_b_inter = None
            self.cond_nl_inter = nn.Identity()
        ...
        if dim_cond > 0 and use_affine:
            ...
            self.cond_a_inter = None

        if dim_cond > 0 and use_adain:
            self.cond_in_intra = nn.InstanceNorm1d(out_channels)
            self.cond_in_inter = nn.InstanceNorm1d(out_channels)
        else:
    ...
    def forward(self, x, C=None):
        ...
        ---------
        x : torch.Tensor
            Input tensor of dimension [B_size*spks, N, K, S].
        C : torch.Tensor
            Input tensor of condition [B_size*spks, N_c].

        Return
        ---------
        out: torch.Tensor
            Output tensor of dimension [B_size*spks, N, K, S].
            where, B = Batchsize,
               N = number of filters
        ...
        # [B/G, S, K, NG]
        intra = intra.view(B // Group, S, K, N * Group)
        intra = self.cond_intra(intra, C=C)
        ...
        # [BK/G, S, NG]
        inter = (
            intra.permute(0, 2, 3, 1)
            .contiguous()
            .view(B * K // Group, S, N * Group)
        )
        # [BK/G, S, NG]
        ...
        # [B/G, K, S, NG]
        inter = inter.view(B // Group, K, S, N * Group)
        inter = self.cond_inter(inter, C=C)
    ...
    def cond_intra(self, X, C=None):
        if C is None:
            ...
            X = self.cond_in_intra(X.transpose(1, 3))
            X = self.cond_nl_intra(Ca * X + Cb)
            ...
            return self.cond_nl_intra(Ca * X + Cb)

    def cond_inter(self, X, C=None):
        if C is None:
            return X
            ...
            X = self.cond_in_inter(X.transpose(1, 3))
            X = self.cond_nl_inter(Ca * X + Cb)
            return X.transpose(1, 3).contiguous()
        else:
            Batch = C.size(0)
            Ca = (
...
class DualPath_Model(nn.Module):
    ...
    def forward(self, x, C=None):
        ...
        ---------
        x : torch.Tensor
            Input tensor of dimension [B*spks, N, L].
        C : torch.Tensor
            Input tensor of condition [B*spks, N_c].

        Returns
        -------
        out : torch.Tensor
            Output tensor of dimension [B*spks, N, L]
...
class SpkResNet(nn.Module):
    def __init__(
        self,
        in_channels=[257, 512, 256],
        ...
        self.post_net = nn.Sequential(
            PoolingLayer(z_channels, **pooling_param),
            nn.ReLU(),
            nn.Linear(
                z_channels, z_channels * 2 if variational else z_channels
            ),
        )
...
class PoolingLayer(nn.Module):
    def __init__(
        self, input_dim, hidden_dim=None, attention=False, statistic=True
    ):
    ...
    def forward(self, x):
        if self.attention:
            ...
            if self.statistic:
                N = x.size(-1)
                means = (x * w).sum(dim=-1)
                diff = x.pow(2) - means.pow(2)[..., None]
                stds = (diff * w).sum(dim=-1).mul(N / (N - 1)).sqrt()
                x_pooled = torch.cat([stds, means], dim=1)
            else:
                x_pooled = (x * w).sum(dim=-1)
        else:
            if self.statistic:
                stds = torch.std(x, dim=-1)
                means = torch.mean(x, dim=-1)
                x_pooled = torch.cat([stds, means], dim=1)
                # x_pooled = torch.cat(torch.std_mean(x,dim=-1),dim=1)
            else:
...
class SpkClassifier(nn.Module):
    def __init__(self, emb_dim=512, total_spk=101):
        super(SpkClassifier, self).__init__()
        self.emb_dim = emb_dim
        self.total_spk = total_spk

        self.nnet = nn.Sequential(
            nn.ReLU(),
            nn.Linear(emb_dim, emb_dim),
            nn.ReLU(),
            nn.Linear(emb_dim, total_spk),
        )
