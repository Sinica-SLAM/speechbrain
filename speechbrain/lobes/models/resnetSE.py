...
class ResNetSE(nn.Module):
    def __init__(self, block, layers, num_filters, encoder_out, encoder_type='SAP', n_mels=40, log_input=True, **kwargs):
        ...
        self.conv1 = nn.Conv1d(encoder_out, num_filters[0] , kernel_size=3, stride=1, padding=1)
...
class AutoEncoder_SE(nn.Module):
    def __init__(self, layers=[3,6,4,3], num_filters=[256, 128, 64, 32], encoder_out=256):
...
class ResNetSE_trans(nn.Module):
    def __init__(self, block, layers, num_filters, encoder_out, encoder_type='SAP', n_mels=40, log_input=True, **kwargs):
        ...
        self.conv1 = nn.ConvTranspose1d(num_filters[3], encoder_out, kernel_size=3, stride=1, padding=1)
        self.relu = nn.ReLU(inplace=True)
        self.bn1 = nn.BatchNorm1d(num_filters[0])
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            upsample = nn.Sequential(
                nn.ConvTranspose1d(self.inplanes, planes * block.expansion,
                          kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm1d(planes * block.expansion),
            )
...
class SEBasicBlock_trans(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, reduction=8):
        ...
        self.conv1 = nn.ConvTranspose1d(inplanes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = nn.BatchNorm1d(planes)
        self.conv2 = nn.ConvTranspose1d(planes, planes, kernel_size=3, padding=1, bias=False)
        self.bn2 = nn.BatchNorm1d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.se = SELayer(planes, reduction)
        self.downsample = downsample
