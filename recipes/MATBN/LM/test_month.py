...
def dataio_prepare(hparams):
    ...
    data_folder = "results/prepare_cna_month"
    datasets = {}
...
if __name__ == "__main__":
    ...
    for month_name in sorted(datasets.keys()):

        month_loss = lm_brain.evaluate(
            datasets[month_name],
            min_key="loss",
            test_loader_kwargs=hparams["test_dataloader_opts"],
        )
        months.append(month_name)
        loss.append(month_loss)
    pd.DataFrame({"month": months, "loss": loss}).to_csv(
        os.path.join(hparams["output_folder"], "month_loss.csv"), index=False
    )
