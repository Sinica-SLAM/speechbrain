...
class ST(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        # The input sizes corrspongin to ilens in ESPnet
        feature_sizes = torch.round(wav_lens * feats.shape[1]).int()
        phone_feature_sizes = batch.allosaurus_length
        ...
        if stage == sb.Stage.TEST:
            ...
            hyps = []
            for feat, allosaurus_token, group_id, score in zip(
                feats,
                allosaurus_tokens,
                allosaurus_group_id,
                allosaurus_global_scores,
            ):
                with torch.no_grad():
                    top_b_hyps = self.hparams.Transformer.translate(
                        feat,
                        allosaurus_token.unsqueeze(0),
                        args,
...
def dataio_prepare(hparams):
    ...
    def allosaurus_text_pipeline(allosaurus):
        ...
        yield allosaurus
        tokens_list = hparams["phone_tokenizer"].encode_as_ids(allosaurus)
        yield tokens_list
        tokens_list = (
            [hparams["bos_index"]] + tokens_list + [hparams["eos_index"]]
        )
        tokens = torch.LongTensor(tokens_list)
        yield tokens
        length_of_phone_sequence = len(tokens_list)
        yield length_of_phone_sequence
        tokens = hparams["phone_tokenizer"].encode_as_pieces(allosaurus)
        sequence = "<s> " + " ".join(tokens) + " </s>"
