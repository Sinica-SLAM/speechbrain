...
def prepare_wsj0(
    ...
    msg = "\tCreating csv file for the WSJ0 Dataset.."
    logger.info(msg)
    ...
    if "train" in splits:
        # wav_lst_train = _get_utt_lists(
        #     data_folder, "train", verification_pairs_file, split_speaker
        # )
        prepare_wsj_csv(
    ...
    if "dev" in splits:
        # wav_lst_dev = _get_utt_lists(
        #     data_folder, "dev", verification_pairs_file, split_speaker
        # )
        prepare_wsj_csv(
...
def prepare_dpfn(
    ...
    save_csv_dev = os.path.join(save_folder, 'dev_dpfn.csv')
    save_csv_test = os.path.join(save_folder, 'test_dpfn.csv')
    ...
    msg = "\tCreating csv file for the DPFN Dataset.."
    logger.info(msg)
    ...
    # Creating csv file for training data
    if "train" in splits:
        wav_lst_train = _get_dpfn_utt_lists(
            data_folder, "train"
        )
        prepare_dpfn_csv(
            seg_dur, wav_lst_train, save_csv_train, random_segment, amp_th
    ...
    if "dev" in splits:
        wav_lst_dev = _get_dpfn_utt_lists(
            data_folder, "dev"
        )
        prepare_dpfn_csv(
            seg_dur, wav_lst_dev, save_csv_dev, random_segment, amp_th
    ...
    # For PLDA verification
    if "test" in splits:
        wav_lst_test = _get_dpfn_utt_lists(
            data_folder, "test"
        )
        prepare_dpfn_csv(
...
def _get_utt_split_lists(
    ...
    for data_folder in data_folders:
        ...
        path = os.path.join(data_folder, "si_tr_s", "**", "*.wav")
        if split_speaker:
            ...
            for f in glob.glob(path, recursive=True):
                try:
                    spk_id = f.split("/si_tr_s/")[1].split("/")[0]
                except ValueError:
...
def _get_wsj_utt_lists(
    ...
    for data_folder in data_folders:
        ...
        if split == "train":
            ...
            wav_dirs = ["si_dt_05", "si_et_05"]
        else:
            raise NotImplementedError(
                f"We didn't implement {split}, we only have train & dev"
            )

        for wav_dir in wav_dirs:
            path = os.path.join(data_folder, wav_dir, "**", "*.wav")

            audio_files_list = []
            for f in glob.glob(path, recursive=True):
                # try:
                #     wav_dir_name = "/" + wav_dir + "/"
                #     spk_id = f.split(wav_dir_name)[1].split("/")[0]
                # except ValueError:
                #     logger.info(f"Malformed path: {f}")
                #     continue
                audio_files_list.append(f)
...
def prepare_wsj_csv(seg_dur, wav_lst, csv_file, random_segment=False, amp_th=0):
    """
    Creates the csv file given a list of wav files.
...
def prepare_dpfn_csv(seg_dur, wav_lst, csv_file, random_segment=False, amp_th=0):
    """
    Creates the csv file given a list of wav files.
    ...
    for wav_file in tqdm(wav_lst, dynamic_ncols=True):
        ...
        try:
            ...
            logger.info(f"Malformed path: {wav_file}")
            continue
        audio_id = my_sep.join([ex_id, utt_id.split(".")[0]])

        wav_details = wav_file.split('/')
        metrics_path = wav_details[:-1] + ['metrics.json']
        metrics_path = '/'.join(metrics_path)
