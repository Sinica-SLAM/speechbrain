...
def prepare_wsjmix(
    ...
    if "extr" in datapath:
        create_extr_csv(datapath, savepath)
    
    elif "wsj" in datapath or "WSJ" in datapath:
...
def create_extr_csv(datapath, savepath):
    """
    This function creates the csv files to get the speechbrain data loaders for the wsj0-2mix dataset.
    ...
    """
    for set_type in ["tr", "cv", "tt"]:
        mix_path = os.path.join(datapath, "wav8k/max/" + set_type + "/mix_100/")
        s1_path = os.path.join(datapath, "wav8k/max/" + set_type + "/s1/")
        aux_path = os.path.join(datapath, "wav8k/max/" + set_type + "/aux/")
        aux_xvec_path = os.path.join(datapath, "wav8k/max/" + set_type + "/aux_xvec_pretrain/")

        files = os.listdir(mix_path)
        files.sort()
        vec_files = os.listdir(aux_xvec_path)
        vec_files.sort()

        mix_fl_paths = [mix_path + fl for fl in files]
        s1_fl_paths = [s1_path + fl for fl in files]
        aux_fl_paths = [aux_path + fl for fl in files]
        aux_xvec_fl_paths = [aux_xvec_path + fl for fl in vec_files]
        ...
        with open(savepath + "/wsj_extr_" + set_type + ".csv", "w") as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=csv_columns)
            writer.writeheader()
            for i, (mix_path, s1_path, aux_path, aux_xvec_path) in enumerate(
                zip(mix_fl_paths, s1_fl_paths, aux_fl_paths, aux_xvec_fl_paths)
            ):
